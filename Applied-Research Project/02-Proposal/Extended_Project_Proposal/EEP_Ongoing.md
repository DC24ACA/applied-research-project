...
2. Develop a secure, isolated Wazuh and Sysmon environment, generate selected controlled attack scenarios, and establish independent ground truth for both self-generated and Windows-APT attack scenarios.

3. Provide selected LLMs with the same prepared Wazuh and Sysmon evidence for each scenario and the same investigation instructions, requiring them to autonomously reconstruct each incident and produce a structured report.
...
A small number of controlled attack scenarios will be generated in the isolated Windows environment using Atomic Red Team, with Wazuh and Sysmon collecting the resulting evidence. More complex scenarios will be selected from the Windows APT dataset. For self-generated scenarios, ground truth will be established from the known actions and techniques executed. For Windows APT scenarios, ground truth will be established from the dataset documentation and scenario mappings. Ground-truth information, scenario identifiers and explicit ATT&CK labels will be removed from the evidence provided to the LLMs. Okuma et al. (2023) used Atomic Red Team with Sysmon to generate known ATT&CK-aligned activity, showing how controlled attacks can provide a consistent basis for evaluation.

For each scenario, every selected LLM will receive the same prepared evidence, investigation instructions and report structure. The model and version number and experimental conditions will be recorded to support reproducibility. Kramer et al. (2025) used consistent evidence and fixed prompts across experimental conditions, supporting the use of standardised inputs when comparing LLM outputs. Each LLM will be required to identify what happened, reconstruct the sequence of activity and support its conclusions using the supplied evidence. Autonomous investigation means that once the evidence and instructions are provided, the LLM must complete the investigation without human assistance.
...
3. **September** – Build and validate the isolated Windows environment, configure Wazuh and Sysmon, generate initial controlled attack scenarios and validate evidence collection.

4. **October** – Select final self-generated and Windows-APT scenarios, prepare security evidence, establish independent ground truth and finalise standardised LLM instructions/report structure.

5. **November** – Conduct autonomous LLM investigations using standardised evidence and record all generated reports.
...
If either evidence source proves unsuitable, greater emphasis may be placed on the remaining source while retaining the same ground-truth and evaluation approach. If technical issues occur, the number of scenarios or models may be reduced while maintaining the core evaluation framework.

# Section 6 – Required Resources and Tools

The project will use VMware Workstation Pro to create a virtual Windows security environment, with Wazuh for security monitoring and alerts, Sysmon for recording Windows activity and Atomic Red Team to generate selected controlled attack scenarios. The Windows APT dataset will provide additional, more complex attack scenarios. Selected LLMs will be used for the investigations and GitHub for documentation and results. The main skills required include configuring the virtual environment, Wazuh and Sysmon, generating controlled attack activity, understanding Windows security logs, mapping activity to MITRE ATT&CK, organising security evidence, creating ground truth and evaluating LLM-generated reports. These skills will be developed through practical testing, official documentation, academic literature and practical guides.
...
This project is not expected to require formal ethics approval because it does not involve human participants or the collection of personal data. The research will use a publicly available Windows APT dataset alongside security logs generated within an isolated Windows environment. All generated attack activity will be conducted against virtual systems owned and controlled by the researcher and will not target real users, organisations or external systems.
